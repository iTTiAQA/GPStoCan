...
static uint8_t g_uart_tx_buf[ATK_MW1278D_UART_TX_BUF_SIZE]; /* ATK-MW1278D UART发送缓冲 */
...
/**
 * @brief       获取ATK-MW1278D UART接收到的一帧数据
 * @param       无
...
void atk_mw1278d_uart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Init(&g_uart_handle);                                  /* 使能ATK-MW1278D UART
 ...
 * @retval      无
 */
void ATK_MW1278D_TIM_IRQHandler_callback(void)//改了个名
{
 ...
 * @retval      无
 */
void ATK_MW1278D_UART_IRQHandler_callback(void)//改了个名
{
    uint8_t tmp;
